    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                cv2.putText(self.im, label, (c1[0], c1[1] - 2), 0, self.lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
###               # uart send
                send_msg = '(' + str(c1[0]) + ',' + str(c1[1]) + ')'
                # uart.Usart_send('(' + str(c1[0]) + ',' + str(c1[1]) + ')')
                ser = uart.ser
